...
fn parse_passport(raw_passport: &str) -> Result<Passport, &str> {
    let mut hash_map = raw_passport.split_whitespace()
        .map(|kv|
            kv.split(":")
            .collect::<Vec<&str>>()
...
impl TryFrom<PartialPassport> for Passport {
    ...
    fn try_from(p: PartialPassport) -> Result<Self, Self::Error> {
        match (p.byr, p.iyr, p.eyr, p.hgt, p.hcl, p.ecl, p.pid) {
            ...
            ) => Ok(Passport { byr, iyr, eyr, hgt, hcl, ecl, pid, cid: p.cid }),
...
fn parse_hgt(hgt: &str) -> Option<Height> {
    ...
    .map(|height| Height { height, unit: unit.to_string() })
}
...
fn parse_hcl(hcl: &str) -> Option<String> {
    ...
    Some(head).filter(|&c| c == "#")
    .and(Some(tail)).filter(|tail| tail.len() == 6 && tail.chars().all(|c| valid_hcls.contains(&c)))
    .map(|_| hcl.to_string())
}

fn parse_ecl(ecl: &str) -> Option<String> {
    //exactly one of: amb blu brn gry grn hzl oth.
    let valid_ecls: HashSet<&str> = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].iter().cloned().collect();
    if valid_ecls.contains(ecl) { Some(ecl.to_string()) }
    else { None }
}

fn parse_pid(pid: &str) -> Option<String> {
    //a nine-digit number, including leading zeroes.
    if pid.len() == 9 && pid.chars().all(char::is_numeric) { Some(pid.to_string()) }
    else { None }
}
