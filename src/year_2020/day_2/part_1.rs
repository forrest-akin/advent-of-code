...
fn parse_input( input: &str ) -> Result<Vec<Password>, &str> {
    ...
    .map(|line|
        ...
        .collect::<Vec<&str>>()
        .try_into()
        .map_err(|_| "ParseError: line format must be `{policy}: {password}`")
...
fn parse_policy( policy: &str ) -> Result<Policy, &str> {
    ...
    .collect::<Vec<&str>>()
    .try_into()
    .map_err(|_| "ParseError: policy format must be `{range} {pattern}`")
...
fn parse_range( range: &str ) -> Result<[i32; 2], &str> {
    ...
    .map_err(|_| "ParseError: range min/max must be integers")
    .and_then(|vec|
        vec.try_into()
        .map_err(|_| "ParseError: range format must be `{min}-{max}`"))
}
