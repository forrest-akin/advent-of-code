...
pub fn main() {
    let raw_input = fs::read_to_string("src/year_2020/day_2/input").expect("IOError: unable to read input");
...
fn parse_line(line: &str) -> Result<Password, &str> {
    line.split(": ")
    .collect::<Vec<&str>>()
    .try_into().map_err(|_| "Invalid Format: line format must be `{policy}: {password}`")
...
fn parse_policy(policy: &str) -> Result<Policy, &str> {
    policy.split(" ")
    .collect::<Vec<&str>>()
    .try_into().map_err(|_| "Invalid Format: policy format must be `{range} {pattern}`")
    .and_then(|[raw_range, pattern]: [&str; 2]| {
        parse_range(raw_range)
        .map(|[min, max]| Policy {
            min,
            max,
            pattern: pattern.chars().nth(0).unwrap(),
        })
    })
}
...
fn parse_range(range: &str) -> Result<[usize; 2], &str> {
    range.split("-")
    .map(str::parse)
    .collect::<Result<_, _>>().map_err(|_| "Invalid Format: range min/max must be integers")
    .and_then(|vec: Vec<usize>| {
        vec.try_into().map_err(|_| "Invalid Format: range format must be `{min}-{max}`")
    })
}
