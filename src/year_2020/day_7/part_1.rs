...
pub fn main() {
    let input =
        fs::read_to_string("src/year_2020/day_7/input").expect("IOError: unable to read input");
...
fn bags_by_children(bag_rules: Vec<BagRule>) -> HashMap<String, HashSet<String>> {
    bag_rules.iter().fold(HashMap::new(), |map, parent| {
        parent
            .contents
            .keys()
            .fold(map, |mut _map, child_bag_type| {
...
fn insert_ancestors(
    container_sets_map: &HashMap<String, HashSet<String>>,
    set: HashSet<String>,
    bag_type: &str,
) -> HashSet<String> {
    match container_sets_map.get(bag_type) {
        Some(container_set) => container_set.iter().fold(set, |s, bag_type| {
...
fn insert_bag(
    ...
    if set.contains(bag_type) {
        return set;
    } else {
        set.insert(bag_type.to_string());
    }
